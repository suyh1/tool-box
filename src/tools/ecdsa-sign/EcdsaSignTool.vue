...
const namedCurve = ref<EcdsaCurve>('P-256')
const hash = ref<EcdsaHash>('SHA-256')
...
const meta = computed(() => `${namedCurve.value} / ${hash.value}`)
...
async function signMessage() {
  ...
  const result = await signEcdsaMessage(message.value, privatePem.value, {
  ...
  liveMessage.value = 'ECDSA 签名已生成'
}

async function verifySignature() {
  resetFeedback()
  isProcessing.value = true
  const result = await verifyEcdsaSignature(message.value, signature.value, publicPem.value, {
    namedCurve: namedCurve.value,
    hash: hash.value,
  })
  isProcessing.value = false
  ...
  liveMessage.value = result.verified ? 'ECDSA 验签通过' : 'ECDSA 验签未通过'
}

async function useSample() {
  resetFeedback()
  isProcessing.value = true
  const keys = await generateEcdsaKeyPair(namedCurve.value)
  isProcessing.value = false
...
function clearAll() {
  ...
  resultOutput.value = ''
  resetFeedback()
  liveMessage.value = 'ECDSA 工作区已清空'
}
...
<template>
  <section class="grid gap-4">
    ...
    <ToolPanel
      title="ECDSA 签名 / 验签"
      description="使用 ECDSA PEM 私钥生成 Base64 签名，并用公钥验证。"
      :meta="meta"
    >
      <template #icon>
      ...
      <ToolActionBar>
        ...
        <template #secondary>
          ...
          >
            <option v-for="item in supportedEcdsaCurves" :key="item" :value="item">
              {{ item }}
            </option>
          </select>
          <select
            v-model="hash"
            aria-label="ECDSA hash"
            class="h-9 rounded-md border border-border bg-background px-3 text-sm text-foreground"
          >
            <option v-for="item in supportedEcdsaHashes" :key="item" :value="item">
              {{ item }}
            </option>
          </select>
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      ...
      <ToolTextareaPanel
        ...
        ariaLabel="ECDSA private key PEM"
      ...
      <ToolTextareaPanel
        ...
        ariaLabel="ECDSA public key PEM"
