...
function encodeInput() {
  resetFeedback()
  output.value = encodeTextToBinary(input.value)
  meta.value = 'Binary 已编码'
  liveMessage.value = meta.value
}

function decodeInput() {
  resetFeedback()
  const result = decodeBinaryToText(input.value)
...
function useSample() {
  ...
  resetFeedback()
  meta.value = '示例已载入'
  liveMessage.value = 'Binary 示例已载入'
}
...
function clearAll() {
  ...
  resetFeedback()
  meta.value = '等待文本'
  liveMessage.value = 'Binary 工作区已清空'
}
...
async function copyOutput() {
  ...
  copied.value = true
  liveMessage.value = 'Binary 输出已复制'
}
</script>
...
<template>
  <section class="grid gap-4">
    ...
    <ToolPanel
      title="Binary 编解码"
      description="在 UTF-8 文本和 8 位二进制字节表示之间转换。"
      :meta="meta"
    >
      <template #icon>
    ...
    <div class="grid gap-4 lg:grid-cols-2">
      <ToolTextareaPanel
        ...
        empty-message="粘贴文本或 8 位二进制字节。解码时可包含空格、冒号、短横线或下划线。"
      />
